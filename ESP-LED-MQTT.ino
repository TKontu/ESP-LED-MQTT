...
void publishRecovery() {
  ...
  //JsonObject& doc = jsonBuffer.createObject();   // Create the JSON object
  ...
  if (rgbValueOne[0] != 0 | rgbValueOne[1] != 0 | rgbValueOne[2] != 0) {
    JsonArray rgbValueOneArray = doc.createNestedArray("1");
  ...
  if (rgbValueTwo[0] != 255 | rgbValueTwo[1] != 255 | rgbValueTwo[2] != 255) {
    JsonArray rgbValueTwoArray = doc.createNestedArray("2");
...
#pragma region commandlTopic
  if (String(commandlTopic).equals(topic)) {
    ...
    Serial.println(topic);  
    if (length == 1) {
      Serial.println("Message is numerical input:");
      byte valprint = (char)payload[0];
      Serial.write(valprint);
      Serial.println();
      // Off
      if ((char)payload[0] == '0') {
        resetGlobalAnimationVariables();
      ...
      Serial.println("Message is numerical JSON input:");
      ...
      DynamicJsonDocument doc(1024);                            //ArduinoJson 6 equivalent of next line.
      ...
      //JsonObject& doc = jsonBuffer.parseObject(message_buff); // Step 2: Deserialize the JSON string
      ...
      if (doc.isNull()) {
        ...
        if (doc.containsKey("1")) {
          rgbValueOne[0] = doc["1"][0];
          rgbValueOne[1] = doc["1"][1];
          rgbValueOne[2] = doc["1"][2];
        ...
        if (doc.containsKey("2")) {
          rgbValueTwo[0] = doc["2"][0];
          rgbValueTwo[1] = doc["2"][1];
          rgbValueTwo[2] = doc["2"][2];
                  flipFlop = 0; // When displaying a static colour this is set to 1 once the LED strip has been updated, resetting this to 0 is needed to trigger the strip update again now the colour has changed.
          publishColour();
        }
...
#pragma region recoveryTopic
    ...
    deserializeJson(doc, message_buff);             //deserialize in arduinoJson 6. Same as below line
    //JsonObject& doc = jsonBuffer.parseObject(message_buff);    // Step 2: Deserialize the JSON string
